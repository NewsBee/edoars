...
const EditUserForm = ({ userId, slug }: { userId: number; slug: string }) => {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (response.ok) {
          // Isi data formulir dengan data user yang sudah ada
          const userData = result;
          //   console.log(userData.name)
          setName(userData[0].name);
          setEmail(userData[0].email);
          setNim(userData[0].nim);
          setNip(userData[0].nip);
          setPassword(userData[0].password);
          setStatus(userData[0].status === "1" ? "Aktif" : "Inaktif");
          setSelectedTypes(userData[0].StudentPermissions || []);
        } else {
  ...
  const handleTypeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSelectedTypes((prevSelectedTypes: any) => {
      if (isChecked) {
        // Jika checkbox dicentang, tambahkan ke selectedTypes
        return [...prevSelectedTypes, { typeId }];
      } else {
        // Jika checkbox dibatalkan centangnya, hapus dari selectedTypes
        return prevSelectedTypes.filter(
          (selected: any) => selected.typeId !== typeId,
        );
      }
    });
  };
                    ...
                    {availableTypes.map((type) => (
                      <div key={type.id} className="flex items-center">
                        <input
                          ...
                          id={type.id}
                          value={type.id}
                          // Periksa apakah `selectedTypes` mengandung `typeId`
                          checked={selectedTypes.some(
                            (selected: any) => selected.typeId === type.id,
                          )}
